...
This system operates by harvesting existing ambient radio signals (e.g., WiFi, Cellular, Bluetooth) through the following process:  
...
Since it relies on signal reflection, a backscatter communication environment always involve two coexisting links: 
...
Since the backscatter link is a reflection, both links operate on the same radio frequency range, and their signals may interfere with each other. This problem is known as **co-channel interference**. This project sets out to study the impact of backscatter communications on the exisiting communication (legacy) links.
...
Get the source code:
```bash
git clone git@github.com:jehuddleston/EmbWIoT_backscatter_measurement.git
cd EmbWIoT_backscatter_measurement.git
```
